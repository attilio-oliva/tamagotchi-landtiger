...
// custom functions
void PutCharOnMemoryBuffer(int Xpos, int Ypos, uint8_t ASCI, uint16_t charColor,
                           uint16_t bkColor, uint16_t* memory_buffer,
                           uint8_t buffer_origin_x, uint8_t buffer_origin_y,
                           uint8_t buffer_width, uint8_t buffer_height) {
  uint16_t start_y = (Ypos < 0) ? -Ypos : 0;
  uint16_t start_x = (Xpos < 0) ? -Xpos : 0;

  uint16_t i, j;
  uint8_t buffer[16], tmp_char;
  GetASCIICode(buffer, ASCI);
  for (i = start_y; i < 16 && Ypos + i < buffer_height; i++) {
    tmp_char = buffer[i];
    for (j = start_x; j < 8 && Xpos + j < buffer_width; j++) {
      uint16_t* buffer_pixel =
          memory_buffer + (Ypos + i) * buffer_width + Xpos + j;
      if (((tmp_char >> (7 - j)) & 0x01) == 0x01) {
        *buffer_pixel = charColor;
      } else {
        *buffer_pixel = bkColor;
      }
    }
  }
}

void GUI_TextOnMemoryBuffer(int Xpos, int Ypos, uint8_t* str, uint16_t Color,
                            uint16_t bkColor, uint16_t* memory_buffer,
                            uint8_t buffer_origin_x, uint8_t buffer_origin_y,
                            uint8_t buffer_width, uint8_t buffer_height) {
  int start_x = Xpos;
  ...
  do {
    TempChar = *str++;
    PutCharOnMemoryBuffer(Xpos, Ypos, TempChar, Color, bkColor, memory_buffer,
                          buffer_origin_x, buffer_origin_y, buffer_width,
                          buffer_height);
    if (Xpos < MAX_X - 8 && Xpos < buffer_width - 8) {
      Xpos += 8;
    } else if (Ypos < MAX_Y - 16 && Ypos < buffer_height - 16) {
      Xpos = start_x;
      Ypos += 16;
    } else {
      Xpos = start_x;
